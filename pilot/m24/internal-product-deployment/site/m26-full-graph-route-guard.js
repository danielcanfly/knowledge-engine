(function () {
  ...
  function looksLikeFullGraphRendered() {
  ...
  async function ensureFullGraphSurface() {
    ...
    setTitle();
    if (looksLikeFullGraphRendered()) return;
    try {
      ...
      if (!response.ok || payload.status !== "ok" || !Array.isArray(payload.nodes) || !Array.isArray(payload.edges)) {
        return;
      }
