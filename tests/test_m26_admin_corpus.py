...
def snapshot(*, sources=None, artifacts=None, vectors=None, active="release-2", warnings=None):
    return {"sources": sources or [], "artifacts": artifacts or [], "vectors": vectors or [], "active_release_marker": active, "warnings": warnings or []}
...
def test_duplicate_slug_is_visible_for_both_source_rows():
    rows = reconcile_corpus(snapshot(sources=[{"source_id": "old", "source_path": "posts/a.md"}, {"source_id": "new", "source_path": "drafts/a.md"}]))
...
def test_rename_does_not_silently_collapse_old_artifact_into_new_source():
    ...
    assert "CORPUS_ORPHANED_ARTIFACT" in next(row for row in rows if row["source_id"] == "old")["reasons"]
