...
def test_stateful_coordinator_only_replays_after_success() -> None:
    ...
    with pytest.raises(AdminAPIError) as conflict:
        coordinator.begin_stateful(**{**kwargs, "request_payload": {"confirmation": True}})
...
def test_destructive_preview_does_not_consume_idempotency_key_as_success() -> None:
    ...
    assert first.json()["error"]["code"] == ("ADMIN_INGESTION_DESTRUCTIVE_CONFIRMATION_REQUIRED")
    ...
    assert second.json()["error"]["code"] == ("ADMIN_INGESTION_DESTRUCTIVE_CONFIRMATION_REQUIRED")
