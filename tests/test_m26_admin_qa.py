...
def _event(trace_id: str = "trace-1") -> dict:
    return {
        ...
        "timeline": [
            {"stage": "retrieval", "status": "empty", "latency_ms": 88}
        ],
...
def test_unavailable_source_does_not_fabricate_empty_events() -> None:
    result = UnavailableQaEventSource().list_events(
        event_class=None, release_id=None
    )
...
def test_export_is_deterministic_and_redacts_hostile_header_text() -> None:
    ...
    assert hashlib.sha256(markdown_a.encode()).hexdigest() == hashlib.sha256(
        markdown_b.encode()
    ).hexdigest()
