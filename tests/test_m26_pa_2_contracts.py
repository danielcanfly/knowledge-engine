...
import pytest
from knowledge_engine.m26_real_corpus_binding import CONTRACT_SCHEMA_PATH, ENTRY_PATH, FAILURE_SCHEMA_PATH, POLICY_PATH, RECEIPT_SCHEMA_PATH, REGISTRY_PATH, RealCorpusBindingError, canonical_sha256, load_json, validate_pa2_contracts, verify_self_digest
...
def refresh_registry(root: Path) -> None:
    ...
    registry['artifacts'] = {'entry_contract_sha256': canonical_sha256(entry), 'retrieval_policy_sha256': canonical_sha256(policy)}
...
@pytest.mark.parametrize(('path', 'field'), [(ENTRY_PATH, 'unexpected_entry_field'), (POLICY_PATH, 'unexpected_policy_field'), (REGISTRY_PATH, 'unexpected_registry_field')])
...
def test_g0_status_drift_rejected(tmp_path: Path) -> None:
    ...
    schema['oneOf'][0]['properties']['predecessors']['properties']['g0_acceptance_self_sha256'] = {'const': g0['self_sha256']}
...
def test_m25_pointer_promotion_alone_is_not_closure(tmp_path: Path) -> None:
    ...
    write_json(root / ENTRY_PATH, entry)
    schema = load_json(root / CONTRACT_SCHEMA_PATH)
    schema['oneOf'][0]['properties']['predecessors']['properties']['m25_final_reconciliation_self_sha256'] = {'const': m25['self_sha256']}
    write_json(root / CONTRACT_SCHEMA_PATH, schema)
    refresh_registry(root)
...
def test_read_only_credential_contracts_are_frozen() -> None:
    ...
    assert policy['read_only']['r2']['credential_contract_sha256'] == '28929389da704c879907f66af6934f90e587f3260333bea72d3df4ffb2ffb0e0'
    assert policy['read_only']['qdrant']['credential_contract_sha256'] == 'fbcc36fda02d3b8eaf0af777531ab2c2b7b5c357df72ae3cebe50e44b57dc42f'

def test_authority_grants_only_non_live_repair() -> None:
    entry = load_json(ROOT / ENTRY_PATH)
    authority = entry['authority']
    assert authority['non_live_p0_p1_repair'] is True
    assert all((value is False for key, value in authority.items() if key != 'non_live_p0_p1_repair'))
...
def test_code_only_merge_cannot_accept_pa2() -> None:
    entry = load_json(ROOT / ENTRY_PATH)
    assert entry['acceptance'] == {'code_only_merge_is_acceptance': False, 'live_evidence_required': True, 'daniel_exact_live_run_approval_required': True, 'independent_reconciliation_required': True}
    registry = load_json(ROOT / REGISTRY_PATH)
