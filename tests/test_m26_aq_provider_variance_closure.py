...
def _safe_abstention() -> tuple[dict[str, Any], dict[str, Any]]:
    return (
        {
...
def _install_fake_synthesis(
    monkeypatch: pytest.MonkeyPatch,
    *,
    repaired_answer: str = "A verified cited answer.",
) -> list[dict[str, Any]]:
    repair_calls: list[dict[str, Any]] = []

    def fake_synthesize(**kwargs: Any) -> tuple[dict[str, Any], dict[str, Any]]:
        del kwargs
        return _safe_abstention()

    def fake_repair(**kwargs: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    monkeypatch.setattr(production, "_production_semantic_synthesize", fake_synthesize)
    monkeypatch.setattr(aq_v2_patch, "_runtime_bound_semantic_repair_v2", fake_repair)
    return repair_calls


def test_in_progress_text_does_not_create_persisted_progress_requirement(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        production,
...
def test_graph_variance_repair_requires_exact_precedes_endpoint_proof(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    requirements = [
        _requirement(
    ...
    repair_calls = _install_fake_synthesis(monkeypatch)
    ...
    for invalid in (
        {**exact_endpoint, "matched": False},
        {**exact_endpoint, "relation_type": "depends_on"},
        {**exact_endpoint, "edge_target": ""},
    ):
        repair_calls.clear()
        answer, closure = production._synthesize_with_bounded_provider_variance_repair(
            question="Alpha Note comes before Beta Note in the relation graph. What is recorded?",
...
def test_route_replan_provider_abstention_uses_only_bounded_verified_repair(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    question = (
...
def test_visual_source_authority_variance_has_narrow_repair_shape(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    question = (
    ...
    repair_calls = _install_fake_synthesis(monkeypatch, repaired_answer=text)
    answer, closure = production._synthesize_with_bounded_provider_variance_repair(
        question=question,
        trace_id="trace_visual_authority",
        intent_class="provenance_source_trace",
        evidence=[{"evidence_id": "source_authority"}],
        provider_client=object(),
        requirements=requirements,
        endpoint_proof={},
    )
    assert answer["terminal_status"] == "verified_answer_ready_candidate"
    assert closure["provider_variance_repair_kind"] == "visual_source_authority"
    assert len(repair_calls) == 1


def test_unrelated_provider_abstention_stays_fail_closed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repair_calls = _install_fake_synthesis(monkeypatch)
    requirements = [
...
def test_precedes_repair_guidance_preserves_relation_semantics() -> None:
    guidance = production._production_variance_repair_guidance("M26-PA7-ME-036")
    lowered = guidance.casefold()
    assert "precedes" in lowered
    assert "ordering" in lowered
    assert "dependency" in lowered
    assert "causal" in lowered


def test_existing_hard_verifier_entrypoint_remains_in_runtime_bound_repair() -> None:
