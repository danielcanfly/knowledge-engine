...
def test_daily_reset_boundary_remains_midnight_utc_across_boundary_fixture() -> None:
    ...
    before_payload = TestClient(make_app(before)).get("/v1/admin/usage", headers=headers()).json()
    after_payload = TestClient(make_app(after)).get("/v1/admin/usage", headers=headers()).json()
...
def test_ai_gateway_payload_is_redacted_and_never_trusted_without_source() -> None:
    ...
    unqualified = TestClient(make_app({"ai_gateway": {"value": {"covered_requests": 99}}})).get(
