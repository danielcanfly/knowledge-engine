...
def test_get_projects_git_source_into_canonical_read_envelope() -> None:
    response = TestClient(make_app()).get(
        "/v1/admin/suggested-questions", headers=headers()
    )
    ...
    assert all(
        item["category"] is None and item["tags"] == []
        for item in payload["data"]["questions"]
    )
    ...
    response = TestClient(make_app()).put(
        ...
        headers=headers(
            **{
                ...
                "idempotency-key": "p06-missing-gate-0001",
            }
        ),
...
def test_put_still_blocks_when_capability_is_enabled_but_write_authority_is_unselected() -> None:
    response = TestClient(make_app(publish_enabled=True)).put(
        ...
        headers=headers(
            **{
                "content-type": "application/json",
                "idempotency-key": "p06-write-authority-01",
            }
        ),
        json={
            "base_revision": "github-blob:blob123",
            "operations": [{"op": "disable", "id": "sq_1"}],
        },
    )
    assert response.status_code == 409
    assert (
        response.json()["error"]["code"]
        == "SUGGESTED_QUESTIONS_WRITE_AUTHORITY_UNSELECTED"
    )
