...
class M3ModeProviderClient:
    """Shadow-only M3 client; B disables thinking only for closure calls."""

    def __init__(self, api_key: str, *, closure_thinking_disabled: bool, max_calls: int = 4) -> None:
        if not api_key:
            raise LiveGateError("MINIMAX_API_KEY missing")
        self.api_key = api_key
        self.closure_thinking_disabled = closure_thinking_disabled
    ...
    def call(self, payload: Mapping[str, Any], call_class: str) -> dict[str, Any]:
        if call_class not in CLOSURE_CALLS and call_class != semantic.SEMANTIC_REVIEW_CALL_CLASS:
            raise RuntimeError(f"unexpected Phase 3B provider call class: {call_class}")
        routed_payload = dict(payload)
        if routed_payload.get("model") != MODEL:
            raise RuntimeError("frozen M3 payload model drift")
        thinking_disabled = self.closure_thinking_disabled and call_class in CLOSURE_CALLS
        if thinking_disabled:
            routed_payload["thinking"] = {"type": "disabled"}
        ...
        for network_attempt in range(4):
            ...
            try:
                ...
                if returned_model != MODEL:
                    raise RuntimeError(
                        f"Phase 3B model identity drift expected={MODEL} returned={returned_model}"
                    )
                usage = _usage(body)
                text = _extract_text(body)
                latency_ms = int((time.monotonic() - started) * 1000)
                block_types = _content_block_types(body)
                result = {
                    "text": text,
                    "usage": usage,
                    "cost_usd": "0",
                ...
                self.telemetry.append(
                    {
                        ...
                        "content_block_types": block_types,
                        "closure_thinking_disabled": thinking_disabled,
                    }
                )
                return result
...
def _run_variant(
    *,
    ...
    question: str,
    owner_hash: str,
    closure_thinking_disabled: bool,
) -> dict[str, Any]:
    client = M3ModeProviderClient(
        os.environ.get("MINIMAX_API_KEY", ""),
        closure_thinking_disabled=closure_thinking_disabled,
        max_calls=4,
    )
    started = time.monotonic()
    try:
...
def _initial_closure_blocks(row: Mapping[str, Any]) -> list[str]:
    for item in row.get("provider_calls", []):
        if isinstance(item, Mapping) and item.get("call_class") == "aq_semantic_closure":
            return [str(value) for value in item.get("content_block_types", [])]
    return []
...
def main() -> int:
    ...
    for case_id in SENTINEL_CASE_IDS:
        ...
        a = _run_variant(
            ...
            closure_thinking_disabled=False,
        )
        b = _run_variant(
            root=root,
            gate=gate,
            question=question,
            owner_hash=owner_hash,
            closure_thinking_disabled=True,
        )
        evidence_match = bool(
            not a.get("error")
            and not b.get("error")
        ...
        pairs.append(
            {
                ...
                "a_initial_closure_blocks": _initial_closure_blocks(a),
                "b_initial_closure_blocks": _initial_closure_blocks(b),
            }
        )
    ...
    result = {
        "schema_version": "m26-latency-phase3b-m3-thinking-off-sentinels/v1",
        "frozen_semantic_base_sha": FROZEN_SEMANTIC_SHA,
        "phase2_base_sha": PHASE2_BASE_SHA,
        "model_both_variants": MODEL,
        "a_closure_mode": "m3_default",
        "b_closure_mode": "m3_thinking_disabled",
        "reviewer_mode_both_variants": "m3_frozen_default",
        ...
        "pairs": pairs,
        "authority": "A_frozen_M3_closure_and_frozen_M3_reviewer",
