...
def _required_env(name: str) -> str:
    ...
    if not value:
        raise SystemExit(f"M23.7-R3.8 missing required environment variable: {name}")
    return value


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the untracked M23.7-R3.8 Wrangler placement config"
    )
    ...
    if parsed.scheme != "https" or not parsed.hostname:
        raise SystemExit("M23.7-R3.8 QDRANT_URL must be an HTTPS endpoint")
