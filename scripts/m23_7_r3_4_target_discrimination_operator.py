...
from knowledge_engine.m23_7_r3_4_target_discrimination_repair import (
...
def _required_environment(name: str) -> str:
    ...
    if not value:
        raise SystemExit(f"R3.4 missing environment variable: {name}")
    return value
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the M23.7-R3.4 target-discrimination repair evaluation"
    )
    ...
    candidate = build_repair_candidate(args.evidence_zip)
    sections = [
        SectionInput(
    ...
    report = evaluate_repair_candidate(candidate, query_vectors)

    _write_json(args.candidate_output, redacted_candidate_artifact(candidate))
    _write_json(args.report_output, report)
    print(
        "M23.7_R3.4_TARGET_DISCRIMINATION "
    ...
    return 0 if report["status"] == "pass_target_discrimination_repair" else 30
