...
const REQUEST_SCHEMA = "knowledge-engine-m23-7-r3-worker-request/v1";
const RESPONSE_SCHEMA = "knowledge-engine-m23-7-r3-worker-response/v1";
...
async function timingSafeEqualText(left, right) {
  ...
  for (let index = 0; index < leftDigest.length; index += 1) {
    difference |=
      leftDigest.charCodeAt(index) ^ rightDigest.charCodeAt(index);
  }
  return difference === 0;
}
...
async function validateBody(request) {
  ...
  assertCondition(
    declaredLength > 0 && declaredLength <= MAX_BODY_BYTES,
    "body-size-invalid",
    413,
  );
  ...
  assertCondition(
    body.schema_version === REQUEST_SCHEMA,
  ...
  assertCondition(
    Array.isArray(body.queries) && body.queries.length === SAMPLE_CAP,
    "query-count-drift",
  );
  ...
  for (const item of body.queries) {
    ...
    assertCondition(
      typeof targetSectionId === "string" &&
        ...
        targetSectionId.length <= 300,
      "target-id-invalid",
    );
    assertCondition(
      typeof queryText === "string" &&
        queryText.length > 0 &&
        queryText.length <= 240,
      "query-text-invalid",
    );
    const calculatedDigest = await sha256Hex(
      canonicalDigestInput(probeId, queryText),
    );
    assertCondition(
      await timingSafeEqualText(calculatedDigest, queryDigest),
      "query-digest-drift",
    );
...
function parseEmbeddingRows(payload) {
  const result =
    isObject(payload) && isObject(payload.result) ? payload.result : payload;
...
function validateCollection(payload) {
  assertCondition(
    isObject(payload),
    "collection-response-shape-drift",
    502,
  );
  ...
  assertCondition(
    result.points_count === EXPECTED_POINTS,
    "point-count-drift",
    502,
  );
  assertCondition(isObject(vector), "named-vector-missing", 502);
  assertCondition(
    vector.size === VECTOR_DIMENSION,
    "collection-dimension-drift",
    502,
  );
  assertCondition(
    vector.distance === "Cosine",
    "collection-distance-drift",
    502,
  );
  assertCondition(
...
function validateRankedPoint(raw) {
  ...
  assertCondition(
    typeof payload.section_id === "string" && payload.section_id.length > 0,
  ...
  assertCondition(
    raw.score >= -1 && raw.score <= 1,
    "ranked-score-range-drift",
    502,
  );
...
function parseBatchResults(payload) {
  assertCondition(
    isObject(payload) && Array.isArray(payload.result),
    "batch-response-shape-drift",
    502,
  );
  assertCondition(
    payload.result.length === SAMPLE_CAP,
    "batch-result-count-drift",
    502,
  );
  return payload.result.map((item) => {
    assertCondition(
      isObject(item) && Array.isArray(item.points),
    ...
    const sectionIds = ranked.map((item) => item.sectionId);
    assertCondition(
      sectionIds.length === new Set(sectionIds).size,
      "duplicate-ranked-section",
      502,
    );
    return sectionIds;
  });
...
async function executeObservation(env, validated, now = () => performance.now()) {
  ...
  assertCondition(
    vectors.length === SAMPLE_CAP,
    "embedding-count-drift",
    502,
  );
...
async function handleRequest(request, env) {
  try {
    ...
    assertCondition(
      url.pathname === "/v1/m23-7-r3/observe",
      "route-not-found",
      404,
    );
    assertCondition(
      typeof env.M23_R3_OPERATOR_TOKEN === "string" &&
        env.M23_R3_OPERATOR_TOKEN.length >= 32,
      "operator-secret-missing",
      500,
    );
    const authorization = request.headers.get("authorization") || "";
    const expected = `Bearer ${env.M23_R3_OPERATOR_TOKEN}`;
    assertCondition(
