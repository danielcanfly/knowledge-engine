...
'''
    ...
    p.write_text(s.replace(func_anchor, funcs + func_anchor, 1))
...
def preserve_current_main_test_authority(root: Path) -> None:
    ...
    if stale.exists():
        # The merge stages this candidate-only historical test. Current main has no such path,
        # so force-removal here is an explicit preservation decision, not an accidental deletion.
        run("git", "rm", "-f", "tests/test_m26_sm95_release_runtime_contract.py")
...
dto = {"provider_routing": {"closure_provider_final": "cloudflare", "fallback_used": False,
"fallback_reason": "NONE", "provider_attempts": [
{"provider": "cloudflare", "model": "@cf/openai/gpt-oss-120b", "call_class": "aq_semantic_closure", "latency_ms": 10},
{"provider": "minimax-m3", "model": "MiniMax-M3", "call_class": "aq_claim_semantic_entailment", "latency_ms": 5}]}}
...
def main() -> int:
    ...
    public_tests = [p for p in public_tests if (root / p).exists()]
    if not public_tests:
    ...
    lint_paths = [p for p in lint_paths if (root / p).exists()]
    run("ruff", "check", "--select", "E,F,I", "--ignore", "E501", *lint_paths)

    run("docker", "build", "-t", "l3b-preservation-successor:qualify", ".")
    run("docker", "inspect", "l3b-preservation-successor:qualify", "--format",
