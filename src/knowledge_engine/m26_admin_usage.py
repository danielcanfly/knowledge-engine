...
def _freshness(value: Any) -> str:
    if isinstance(value, str) and value in _ALLOWED_FRESHNESS:
        return value
    return "unknown"
...
def _normalize_limit(
    raw: Any, definition: MetricDefinition
) -> dict[str, Any] | None:
...
def _normalize_metric(definition: MetricDefinition, raw: Any) -> dict[str, Any]:
    ...
    if (
        ...
        return _unavailable_metric(
            ...
            (
                "Telemetry was rejected because value, source, unit, or window "
                "was not trustworthy."
            ),
        )
    ...
    if observed_at is None or freshness == "unknown" or incomplete_coverage:
        ...
        detail = (
            "Usage is reported, but freshness, observation time, or time-window "
...
def _workers_ai_policy() -> dict[str, Any]:
    return {
        "availability": _availability(
            ...
            None,
            (
                "Verified Workers AI free allocation policy snapshot; this is "
                "allocation, not live usage."
            ),
        ),
...
def _unavailable_policy() -> dict[str, Any]:
    return {
        ...
        "provenance": _provenance(
            "rate_limit_policy_source_unavailable", observed_at=None
        ),
        "observed_at": None,
        "freshness": "unknown",
        "limits": None,
...
def _normalize_policy(raw: Any) -> dict[str, Any]:
    ...
    status = (
        "available"
        if observed_at is not None and freshness != "unknown"
        else "partial"
    )
    detail = (
        "Qualified current rate-limit policy."
        if status == "available"
        else "Policy is present but freshness evidence is incomplete."
    )
    return {
        "availability": _availability(
            status,
            None
            if status == "available"
            else "USAGE_RATE_LIMIT_POLICY_PARTIAL_EVIDENCE",
            detail,
        ),
        "provenance": _provenance(source.strip(), observed_at=observed_at),
        "observed_at": observed_at,
...
def _unavailable_gateway() -> dict[str, Any]:
    return {
        "availability": _availability(
            ...
            (
                "AI Gateway billing/coverage telemetry is not wired; traffic "
                "coverage is not inferred."
            ),
        ),
        "provenance": _provenance(
            "ai_gateway_usage_source_unavailable", observed_at=None
        ),
        "observed_at": None,
        "freshness": "unknown",
        "value": None,
...
def _normalize_gateway(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, Mapping):
    ...
    detail = (
        "Qualified AI Gateway usage coverage observation."
        if status == "available"
        else "AI Gateway evidence is present but freshness is incomplete."
    )
    return {
        "availability": _availability(
            status,
            None if status == "available" else "USAGE_AI_GATEWAY_PARTIAL_EVIDENCE",
            detail,
        ),
        "provenance": _provenance(source.strip(), observed_at=observed_at),
        "observed_at": observed_at,
        "freshness": freshness,
        "value": redact(raw.get("value")),
    }
...
def _aggregate_availability(metrics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if states and all(state == "available" for state in states):
        return _availability(
            "available", None, "All required usage metrics are available."
        )
    if any(state in {"available", "partial"} for state in states):
        return _availability(
            ...
            (
                "Partial usage telemetry: one or more required metrics are "
...
def build_usage_payload(
    request: Request, provider: UsageProvider | None = None
) -> dict[str, Any]:
    ...
    policy_raw = raw.get("rate_limit_policy") if isinstance(raw, Mapping) else None
    gateway_raw = raw.get("ai_gateway") if isinstance(raw, Mapping) else None
    return {
        ...
        "data": {
            "metrics": metrics,
            "rate_limit_policy": _normalize_policy(policy_raw),
            "workers_ai_policy": _workers_ai_policy(),
            "ai_gateway": _normalize_gateway(gateway_raw),
        },
    }
...
def install_admin_usage(
    app: FastAPI, *, provider: UsageProvider | None = None
) -> FastAPI:
