...
def _normalize_policy(raw: Any) -> dict[str, Any]:
    ...
    if not isinstance(source, str) or not source.strip() or not isinstance(limits, Mapping):
        return _unavailable_policy()
    safe_limits = {
        key: value for key, value in limits.items() if isinstance(key, str) and _valid_number(value)
    }
    if not safe_limits:
        return _unavailable_policy()
...
def _latest_observed_at(metrics: list[dict[str, Any]]) -> str | None:
    observations = [
        metric["observed_at"] for metric in metrics if isinstance(metric.get("observed_at"), str)
    ]
    if not observations:
        return None
    return max(
