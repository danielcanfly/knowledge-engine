...
def _normalize_observation(definition: DependencyDefinition, raw: Any) -> dict[str, Any]:
...
def _backend_observation(request: Request, observed_at: str) -> dict[str, Any]:
    runtime_identity = next(
        (value for key in _SAFE_RUNTIME_IDENTITY_ENV if (value := os.environ.get(key, "").strip())),
        None,
    )
    return {
        "status": "healthy",
...
def _observer_observations(request: Request, observer: HealthObserver | None) -> Mapping[str, Any]:
...
def install_admin_health(app: FastAPI, *, observer: HealthObserver | None = None) -> FastAPI:
