...
RAW_KEY_PATTERN = re.compile(
    "(?i)(^|[_-])(text|body|content|excerpt|prompt|markdown|document|chunk|passage|raw|html)([_-]|$)"
)
SECRET_KEY_PATTERN = re.compile(
    "(?i)(api[_-]?key|secret|password|authorization|bearer|access[_-]?token|credential|private[_-]?key)"
)
SECRET_VALUE_PATTERNS = (
    re.compile("(?i)\\bbearer\\s+[a-z0-9._~+/=-]{8,}"),
    re.compile("\\bsk-[A-Za-z0-9_-]{16,}\\b"),
    re.compile("\\bgh[pousr]_[A-Za-z0-9_]{20,}\\b"),
    re.compile("\\beyJ[A-Za-z0-9_-]{10,}\\.[A-Za-z0-9_-]{10,}\\.[A-Za-z0-9_-]{8,}\\b"),
)
...
class RealCorpusBindingError(IntegrityError):
    def __init__(
        self, code: str, message: str, *, category: str = "integrity", retryable: bool = False
    ) -> None:
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        return _decode_json_bytes(
            path.read_bytes(), label=path.as_posix(), maximum_bytes=4 * 1024 * 1024
        )
...
def _validate_predecessors(root: Path, entry: Mapping[str, Any]) -> None:
    ...
    expected = {
        ...
        "g0_acceptance_self_sha256": g0["self_sha256"],
        "m25_status": "m25_closed",
        "m25_final_reconciliation_self_sha256": m25["self_sha256"],
        "m25_final_reconciliation_merge_sha": m25["closure_pr"]["merge_sha"]
        if isinstance(m25.get("closure_pr"), dict)
        else "",
    }
    ...
    if (
        g0.get("status") != expected["g0_status"]
        or canonical_statuses.get("g0") != expected["g0_status"]
        or canonical_statuses.get("pa1") != expected["pa1_status"]
    ):
...
def _validate_registry(
    root: Path, *, entry: Mapping[str, Any], policy: Mapping[str, Any], registry: Mapping[str, Any]
) -> None:
    ...
    expected_schemas = {
        "contracts_schema_sha256": sha256_bytes((root / CONTRACT_SCHEMA_PATH).read_bytes()),
        "receipt_schema_sha256": sha256_bytes((root / RECEIPT_SCHEMA_PATH).read_bytes()),
        "failure_schema_sha256": sha256_bytes((root / FAILURE_SCHEMA_PATH).read_bytes()),
    }
...
def validate_pa2_contracts(root: Path) -> dict[str, Any]:
    ...
    for label, schema in (
        ("contracts schema", contract_schema),
    ...
    return {
        "schema_version": "knowledge-engine-m26-pa-2-non-live-evidence/v1",
        "status": "m26_pa_2_non_live_repair_contracts_valid",
        "stage_id": "M26.PA.2",
        "accepted": False,
        ...
        "entry_contract_sha256": canonical_sha256(entry),
        "retrieval_policy_sha256": canonical_sha256(policy),
        "contract_registry_sha256": canonical_sha256(registry),
        "receipt_schema_sha256": sha256_bytes((root / RECEIPT_SCHEMA_PATH).read_bytes()),
        "failure_schema_sha256": sha256_bytes((root / FAILURE_SCHEMA_PATH).read_bytes()),
        "payload_field_count": len(allowlist),
        "expected_point_count": policy["qdrant"]["expected_point_count"],
        "legacy_candidate_merged": False,
    }
...
def _credential_contract_digest(
    *, surface: str, operations: Sequence[str], secret_names: Sequence[str]
) -> str:
...
def _assert_read_only_surface(
    subject: Any,
    *,
    label: str,
    expected_capabilities: frozenset[str],
    expected_credential_digest: str,
) -> None:
    capabilities = getattr(subject, "capabilities", None)
    if capabilities != expected_capabilities:
        raise _failure(
            "M26-PA2-021", f"{label} read-only capability mismatch", category="authority"
        )
    ...
    for name in MUTATION_METHOD_NAMES:
        if callable(getattr(subject, name, None)):
            raise _failure(
                "M26-PA2-024", f"{label} exposes a mutation method", category="authority"
            )
...
class Boto3ReadOnlyObjectStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.credential_contract_sha256 = _credential_contract_digest(
            surface="r2",
            operations=("get",),
            secret_names=("R2_ACCESS_KEY_ID_READ", "R2_SECRET_ACCESS_KEY_READ"),
        )
        self._bucket = bucket
        self._client = boto3.client(
            "s3",
            endpoint_url=_safe_url(endpoint_url, label="R2 endpoint"),
...
class HttpxReadOnlyQdrantClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.credential_contract_sha256 = _credential_contract_digest(
            surface="qdrant",
            operations=("count", "scroll"),
            secret_names=("QDRANT_READ_ONLY_API_KEY",),
        )
    ...
    def _request(
        self, *, collection: str, operation: str, body: Mapping[str, Any], timeout_seconds: float
    ) -> ReadResponse:
        ...
        while True:
            ...
            try:
                ...
                if attempts <= self._maximum_retries:
                    ...
                    continue
                raise _failure(
                    "M26-PA2-028", "Qdrant read timed out", category="transport", retryable=True
                ) from exc
            except httpx.HTTPError as exc:
                raise _failure(
                    "M26-PA2-029", "Qdrant transport failed", category="transport", retryable=False
                ) from exc
            ...
            if response.status_code < 200 or response.status_code >= 300:
                raise _failure(
                    "M26-PA2-031",
                    "Qdrant returned a non-success status",
                    category="transport",
                    retryable=False,
                )
            try:
    ...
    def count(
        ...
        return self._request(
            ...
            operation="count",
            body={"exact": True, "filter": dict(query_filter)},
    ...
    def scroll(
        ...
        return self._request(
            collection=collection, operation="scroll", body=request, timeout_seconds=timeout_seconds
        )


def _qdrant_filter(values: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "must": [{"key": key, "match": {"value": value}} for key, value in sorted(values.items())]
    }
...
def _scan_credential_url(value: str) -> None:
    ...
    try:
        ...
        raise _failure(
            "M26-PA2-033", "payload contains a malformed URL", category="security"
        ) from exc
    sensitive_query = any(
        (
    ...
    if parsed.username is not None or parsed.password is not None or sensitive_query:
        raise _failure(
            "M26-PA2-034", "payload contains a credential-bearing URL", category="security"
        )
...
def _recursive_payload_scan(value: Any, *, key: str | None = None) -> None:
    if key is not None:
        ...
        if key != "text_sha256" and RAW_KEY_PATTERN.search(key):
            raise _failure(
                "M26-PA2-036", "payload contains a raw-text-like key", category="security"
            )
    ...
    if isinstance(value, str):
        ...
        if any(pattern.search(value) for pattern in SECRET_VALUE_PATTERNS):
            raise _failure(
                "M26-PA2-039", "payload contains a secret-like value", category="security"
            )


def _validate_artifact_inventory(
    manifest: Mapping[str, Any], policy: Mapping[str, Any], release_id: str
) -> tuple[int, str]:
...
def _validate_pointer(*, pointer_bytes: bytes, entry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required = {
        ...
        "source_candidate_channel",
        "source_candidate_manifest_sha256",
        "production_authority",
        "public_production_traffic_mutated",
    }
...
def _validate_manifest(
    *, manifest_bytes: bytes, entry: Mapping[str, Any], policy: Mapping[str, Any]
    ...
    manifest = _decode_json_bytes(
        manifest_bytes, label="production manifest", maximum_bytes=4 * 1024 * 1024
    )
    required = {
        "schema_version",
        "release_id",
    ...
    artifact_count, artifact_inventory_sha256 = _validate_artifact_inventory(
        manifest, policy, identity["release_id"]
    )
...
def _validate_qdrant_envelope(value: Mapping[str, Any], operation: str) -> dict[str, Any]:
    _strict_keys(
        value,
        label=f"Qdrant {operation} response",
        required={"status", "result"},
        allowed={"status", "result", "time", "usage"},
    )
...
def _sample_entry(point_id: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "section_id_sha256": sha256_bytes(str(payload["section_id"]).encode("utf-8")),
        "source_id_sha256": sha256_bytes(str(payload["source_id"]).encode("utf-8")),
        "article_id_sha256": sha256_bytes(str(article_id).encode("utf-8"))
        if article_id is not None
        else None,
        "text_sha256": payload["text_sha256"],
        "payload_identity_sha256": canonical_sha256(payload),
    }
...
def _validate_workflow(workflow: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not all(
        isinstance(workflow.get(key), str) and 0 < len(workflow[key]) <= 256
        for key in ("workflow_name", "run_id", "run_attempt", "query_id")
    ):
...
def bind_real_corpus(
    *,
    ...
    _assert_read_only_surface(
        store,
        label="R2 object store",
        expected_capabilities=frozenset(read_only["r2"]["allowed_operations"]),
        expected_credential_digest=read_only["r2"]["credential_contract_sha256"],
    )
    _assert_read_only_surface(
        qdrant,
        label="Qdrant client",
        expected_capabilities=frozenset(read_only["qdrant"]["allowed_operations"]),
        expected_credential_digest=read_only["qdrant"]["credential_contract_sha256"],
    )
    ...
    manifest, artifact_count, artifact_inventory_sha256 = _validate_manifest(
        manifest_bytes=manifest_bytes, entry=entry, policy=policy
    )
    ...
    while True:
        ...
        request: dict[str, Any] = {
            "filter": query_filter,
            "limit": qdrant_policy["page_size"],
            "with_payload": qdrant_policy["with_payload"],
            "with_vector": False,
        }
        ...
        pagination_trace.append(
            {
                "page": page_count,
                "request_offset_sha256": canonical_sha256(offset) if offset is not None else None,
                "row_count": len(rows),
                "next_offset_sha256": canonical_sha256(next_offset)
        ...
        for raw_row in rows:
            ...
            payload = _validate_payload(
                _object(row.get("payload"), "Qdrant payload"), policy=policy
            )
            ...
            rank = canonical_sha256(
                {
                    "point_id_sha256": sample["point_id_sha256"],
                    "payload_identity_sha256": sample["payload_identity_sha256"],
                    "sample_seed": qdrant_policy["sample_seed"],
                }
            )
    ...
    receipt = _receipt_with_digest(
        {
            "schema_version": "knowledge-engine-m26-pa-2-real-corpus-receipt/v1",
            ...
            "qdrant": {
                "collection": identity["qdrant_collection"],
                "filter_sha256": canonical_sha256(query_filter),
                "expected_point_count": qdrant_policy["expected_point_count"],
                "observed_point_count": observed_count,
                "page_size": qdrant_policy["page_size"],
                "page_count": page_count,
                "sample_size": len(samples),
                "sample_seed_sha256": sha256_bytes(qdrant_policy["sample_seed"].encode("utf-8")),
                "pagination_trace_sha256": canonical_sha256(pagination_trace),
                "point_ids_sha256": canonical_sha256(sorted(point_ids)),
                "with_payload": qdrant_policy["with_payload"],
                "with_vector": False,
                "duplicate_point_ids": False,
                "duplicate_section_ids": False,
            },
            "sample": samples,
            "authority": {
                "r2_read_operations": 2,
                "qdrant_count_operations": 1,
                "qdrant_scroll_operations": page_count,
                "qdrant_transport_attempts": count_response.attempts + scroll_attempts,
                "r2_write_operations": 0,
                "qdrant_write_operations": 0,
                "production_pointer_mutations": 0,
                "provider_calls": 0,
                "answer_generation_operations": 0,
                "public_shadow_canary_traffic_operations": 0,
                "vectors_requested": False,
                "vectors_returned": False,
                "raw_text_persisted": False,
                "secrets_persisted": False,
                "read_only_credentials_verified": True,
            },
...
def build_sanitized_failure_receipt(
    *,
    root: Path,
    generated_at: str,
    workflow: Mapping[str, Any],
    ...
    if operation_counts is not None:
        if set(operation_counts) != set(counts) or any(
    ...
    receipt = _receipt_with_digest(
        {
            "schema_version": "knowledge-engine-m26-pa-2-failure-receipt/v1",
            "status": "real_corpus_retrieval_binding_failed",
            "generated_at": generated_at,
            "workflow": workflow_value,
            "error": {
                "code": code,
