...
def _load_object(store: ObjectStore, key: str, label: str) -> tuple[bytes, dict[str, Any]]:
    try:
        data = store.get(key)
    except FileNotFoundError as exc:
        raise M13OperatorError("M13_OPERATOR_OBJECT_MISSING", f"{label} is missing", key=key) from exc
    try:
        value = json.loads(data)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise M13OperatorError("M13_OPERATOR_OBJECT_INVALID", f"{label} is invalid JSON", key=key) from exc
    if not isinstance(value, dict):
        raise M13OperatorError("M13_OPERATOR_OBJECT_INVALID", f"{label} must be an object", key=key)
    return data, value


def load_production_identity(store: ObjectStore) -> tuple[ProductionIdentity, dict[str, Any]]:
    pointer_key = "channels/production.json"
    pointer_bytes, pointer = _load_object(store, pointer_key, "production pointer")
    ...
    if not isinstance(release_id, str) or not RELEASE_ID_RE.fullmatch(release_id):
        raise M13OperatorError("M13_OPERATOR_PRODUCTION_INVALID", "production release_id is invalid")
    if not isinstance(manifest_sha256, str) or not re.fullmatch(r"[a-f0-9]{64}", manifest_sha256):
        raise M13OperatorError("M13_OPERATOR_PRODUCTION_INVALID", "production manifest hash is invalid")
    if not isinstance(manifest_key, str) or not manifest_key:
        raise M13OperatorError("M13_OPERATOR_PRODUCTION_INVALID", "production manifest key is invalid")
    manifest_bytes, manifest = _load_object(store, manifest_key, "production manifest")
...
def _candidate_head(store: ObjectStore, *, default_capacity: int = 2) -> dict[str, Any]:
    ...
    _, head = _load_object(store, CANDIDATE_HEAD_KEY, "candidate concurrency head")
    if head.get("schema_version") != f"{COORDINATOR_SCHEMA}/candidate-head":
        raise M13OperatorError("M13_OPERATOR_CANDIDATE_HEAD_INVALID", "candidate head schema is invalid")
    if not isinstance(head.get("head_version"), int) or not isinstance(head.get("capacity"), int):
        raise M13OperatorError("M13_OPERATOR_CANDIDATE_HEAD_INVALID", "candidate head version or capacity is invalid")
    if not isinstance(head.get("active"), dict):
        raise M13OperatorError("M13_OPERATOR_CANDIDATE_HEAD_INVALID", "candidate active slots are invalid")
    return head


def _batch_views(store: ObjectStore) -> tuple[dict[str, Any], list[tuple[dict[str, Any], Any]]]:
...
def operator_status(
    ...
    *,
    ...
    candidate_head = _candidate_head(store, default_capacity=candidate_capacity)
        ...
        batches.append(
            {
                ...
                "expected_previous_is_current": record.seed.production == production,
    ...
    active_slots = [
        ...
        for number, summary in sorted(candidate_head["active"].items(), key=lambda item: int(item[0]))
...
def stale_report(
    ...
    *,
    ...
    for snapshot, record in batch_values:
        if not snapshot["record"]["terminal"] and record.seed.production != production:
            findings.append(
                {
        ...
        closeout_complete = any(
            item.get("kind") == "closeout" and item.get("state") == "completed"
    ...
    for number, summary in sorted(candidate_head["active"].items(), key=lambda item: int(item[0])):
        if not isinstance(summary, dict) or not isinstance(summary.get("expires_at"), str):
            findings.append(
                {
        ...
        elif parse_utc(summary["expires_at"], "expires_at") < now:
            findings.append(
                {
    ...
    if lease is not None:
        if lease.state in {"active", "permit_issued", "commit_authorized"} and parse_utc(
        ...
        matching = [record for _, record in batch_values if record.batch_id == lease.batch_id]
        if not matching:
            findings.append(
                {
    ...
    findings.sort(key=lambda item: (item["severity"], item["code"], str(item.get("batch_id", ""))))
    return {
    ...
    *,
    ...
    try:
        ...
        checked_objects.update(
            {production_evidence["pointer_key"], production_evidence["manifest_key"]}
        )
    except (M13OperatorError, ValueError) as exc:
        production = None
        issues.append({"code": getattr(exc, "code", "production_invalid"), "message": str(exc)})
    try:
        head, _ = registry._load_head(store)
        checked_objects.add(registry.REGISTRY_HEAD_KEY)
        for batch_id in sorted(head["batches"]):
            try:
                snapshot, _ = registry._load_batch_snapshot(store, head, batch_id)
                checked_objects.add(head["batches"][batch_id]["snapshot_key"])
                ...
                for summary in snapshot["operation_summaries"]:
                    ...
                    if value.get("operation_id") not in {None, summary.get("operation_id")}:
                        issues.append(
                            {
                ...
                issues.append({"code": "batch_integrity_failure", "batch_id": batch_id, "message": str(exc)})
    except Exception as exc:  # fail-closed audit aggregation
        issues.append({"code": "registry_integrity_failure", "message": str(exc)})
    try:
        candidate_head = _candidate_head(store, default_capacity=candidate_capacity)
        if store.head(CANDIDATE_HEAD_KEY) is not None:
            checked_objects.add(CANDIDATE_HEAD_KEY)
        for number, summary in candidate_head["active"].items():
            if not isinstance(summary, dict):
                issues.append({"code": "candidate_slot_invalid", "slot_number": number})
                continue
            key = summary.get("artifact_key")
            if not isinstance(key, str) or store.head(key) is None:
                issues.append({"code": "candidate_slot_artifact_missing", "slot_number": number, "key": key})
                continue
            checked_objects.add(key)
            _, artifact = _load_object(store, key, "candidate slot artifact")
            if artifact.get("slot_id") != summary.get("slot_id"):
                issues.append({"code": "candidate_slot_identity_mismatch", "slot_number": number, "key": key})
    except Exception as exc:  # fail-closed audit aggregation
        issues.append({"code": "candidate_head_integrity_failure", "message": str(exc)})
    try:
        lease, _ = load_production_lease(store)
        if lease is not None:
            ...
            for label, key in (
                ...
                if key is not None:
                    if store.head(key) is None:
                        issues.append({"code": "lease_evidence_missing", "label": label, "key": key})
                    else:
                        checked_objects.add(key)
            if lease.state == "released" and lease.completion_key is None:
                issues.append({"code": "released_lease_missing_completion", "lease_id": lease.lease_id})
    except Exception as exc:  # fail-closed audit aggregation
        issues.append({"code": "production_lease_integrity_failure", "message": str(exc)})
    issues.sort(key=lambda item: (item["code"], str(item.get("batch_id", "")), str(item.get("key", ""))))
    return {
        "schema_version": f"{OPERATOR_SCHEMA}/audit",
        "observed_at": observed_at,
        "passed": not issues,
        "production": production.to_identity() if production is not None else None,
...
def _operation_matches(store: ObjectStore, identity: str) -> list[dict[str, Any]]:
    head, batch_values = _batch_views(store)
...
def operator_lookup(store: ObjectStore, *, identity: str) -> dict[str, Any]:
    if not identity or len(identity) > 220:
        raise M13OperatorError("M13_OPERATOR_LOOKUP_INVALID", "identity is invalid")
    matches: list[dict[str, Any]] = []
    head, batch_values = _batch_views(store)
    if BATCH_ID_RE.fullmatch(identity):
        snapshot, record = registry._load_batch_snapshot(store, head, identity)
        matches.append(
            {
                "kind": "batch",
                "batch_id": identity,
                ...
                matches.append(
                    {
                        ...
                        "state": record.state,
                        "snapshot_key": head["batches"][record.batch_id]["snapshot_key"],
                        "operation_summaries": snapshot["operation_summaries"],
                    }
                )
        ...
        matches.append({"kind": "release_comparison", "comparison_id": identity, "key": key, "value": value})
    elif LEASE_ID_RE.fullmatch(identity):
        lease, _ = load_production_lease(store)
        if lease is not None and lease.lease_id == identity:
            matches.append({"kind": "production_lease", "value": lease.to_dict(), "key": PRODUCTION_LEASE_KEY})
    elif PERMIT_ID_RE.fullmatch(identity):
        key = f"m13/v2/concurrency/production/permits/{identity}.json"
        _, value = _load_object(store, key, "production permit")
        matches.append({"kind": "production_permit", "key": key, "value": value})
    elif AUTHORIZATION_ID_RE.fullmatch(identity):
        key = f"m13/v2/concurrency/production/authorizations/{identity}.json"
        _, value = _load_object(store, key, "commit authorization")
        matches.append({"kind": "commit_authorization", "key": key, "value": value})
    elif COMPLETION_ID_RE.fullmatch(identity):
        key = f"m13/v2/concurrency/production/completions/{identity}.json"
        _, value = _load_object(store, key, "production completion")
        matches.append({"kind": "production_completion", "key": key, "value": value})
    elif SLOT_ID_RE.fullmatch(identity):
        key = f"m13/v2/concurrency/candidate/leases/{identity}.json"
        _, value = _load_object(store, key, "candidate slot")
        matches.append({"kind": "candidate_slot", "key": key, "value": value})
    elif CLOSEOUT_ID_RE.fullmatch(identity):
        key = f"m13/v2/closeouts/{identity}.json"
        _, value = _load_object(store, key, "batch closeout")
        matches.append({"kind": "closeout", "key": key, "value": value})
    elif RELEASE_ID_RE.fullmatch(identity):
        production, evidence = load_production_identity(store)
        if production.release_id == identity:
            matches.append({"kind": "production_release", "production": production.to_identity(), "evidence": evidence})
        for snapshot, record in batch_values:
            ...
            for summary in snapshot["operation_summaries"]:
                ...
                if isinstance(request, dict) and isinstance(evidence_refs, list):
                    for evidence_key in evidence_refs:
                        if not isinstance(evidence_key, str) or store.head(evidence_key) is None:
                            continue
                        _, evidence_value = _load_object(store, evidence_key, "operation evidence")
                        target = evidence_value.get("target_release")
        ...
        raise M13OperatorError("M13_OPERATOR_LOOKUP_TYPE_UNKNOWN", "identity type is not supported")
    if not matches:
        raise M13OperatorError("M13_OPERATOR_LOOKUP_NOT_FOUND", "identity was not found", identity=identity)
    matches.sort(key=lambda item: (item["kind"], str(item.get("batch_id", "")), str(item.get("key", ""))))
    return {
...
def ledger_summary(store: ObjectStore, *, observed_at: str) -> dict[str, Any]:
    parse_utc(observed_at, "observed_at")
    production, _ = load_production_identity(store)
    _, batch_values = _batch_views(store)
    ...
    for snapshot, record in batch_values:
        ...
        for summary in snapshot["operation_summaries"]:
            ...
            operation_counts.setdefault(kind, {})[state] = operation_counts.setdefault(kind, {}).get(state, 0) + 1
            ...
            if kind == "closeout":
                ...
                if isinstance(key, str) and store.head(key) is not None:
                    ...
                    if isinstance(refs, list):
                        for ref in refs:
                            if isinstance(ref, str) and (ref.startswith("issue-") or ref.startswith("ledger")):
        ...
        batches.append(
            {
                ...
                "state": record.state,
                "candidate_channel": record.candidate_channel,
                "expected_previous_release_id": record.seed.production.release_id,
    ...
    normalized_counts = {
        kind: dict(sorted(states.items())) for kind, states in sorted(operation_counts.items())
    }
    return {
        ...
        "production": production.to_identity(),
        "batch_count": len(batches),
        "closed_batch_count": sum(item["state"] == "closed" for item in batches),
        "operation_counts": normalized_counts,
        "batches": batches,
        "closed_batches_missing_ledger_references": sorted(missing_ledger),
