...
_EFFECTIVE_STATES = frozenset({"enabled", "read_only", "disabled", "unavailable", "not_eligible"})
...
def canonicalize_capability(raw: Any) -> dict[str, Any]:
    ...
    capability_id = _safe_string(payload.get("capability_id"), fallback="unknown.capability")
    ...
    if qualification in _QUALIFICATION_STATUSES and effective in _EFFECTIVE_STATES:
        ...
        mutation_authorized = bool(
            mutation_authorized and qualification == "qualified" and effective == "enabled"
        )
    else:
