...
class QaEventSource(Protocol):
    def list_events(
        self, *, event_class: str | None, release_id: str | None
    ) -> QaReadResult: ...
...
def qa_envelope(request: Request, result: QaReadResult) -> dict[str, Any]:
    return {
        ...
        "data": (
            ...
            else redact_qa_value(result.data)
        ),
    }
...
def _router() -> APIRouter:
    ...
    async def export_markdown(
        request: Request, payload: QaExportRequest
    ) -> PlainTextResponse:
        ...
        trace_ids_digest = hashlib.sha256(
            json.dumps(
                sorted(set(payload.trace_ids)), separators=(",", ":")
            ).encode("utf-8")
        ).hexdigest()
        append_audit_event(
            request,
...
def install_admin_qa(
    app: FastAPI, *, source: QaEventSource | None = None
) -> FastAPI:
