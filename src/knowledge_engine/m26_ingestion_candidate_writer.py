    ...
    *,
    ...
    if missing:
        raise CandidateWriteError(
            "candidate runtime artifacts are missing: " + ",".join(missing)
        )
    ...
    if lexical_ids != semantic_ids:
        raise CandidateWriteError(
            "lexical and semantic section_id sets are not exactly equal"
        )

    collection = candidate_qdrant_collection(release)
    keys = {
        kind: f"releases/{release}/artifacts/{kind}.json"
        for kind in sorted(kinds)
    }
    digests = {
        kind: sha256_bytes(artifact_bytes[kind])
        for kind in sorted(kinds)
    }
    manifest_artifacts = [
        {
...
def _verify_vector_result(
    ...
    *,
    ...
    if len(observed) != len(set(observed)):
        raise CandidateWriteError(
            "candidate Qdrant readback contains duplicate section_id"
        )
