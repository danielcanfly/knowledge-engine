...
def router() -> APIRouter:
    ...
    async def inspect_retrieval(payload: PlaygroundRequest, request: Request) -> dict[str, Any]:
        ...
        try:
            result = run_translation_gateway(
                ...
                provider=_translation_provider_for_question(request.app, payload.question),
        ...
        status = "partial" if degraded_reason or not evidence_available else "available"
    ...
    async def full_ask(payload: PlaygroundRequest, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            retrieval_started = "retrieval" in runtime_map
            generation_started = (
                any(item.get("type") == "model.started" for item in events)
        ...
        _set_stage(trace, "evidence_organization", "complete")
        accounting = (
            runtime.get("accounting") if isinstance(runtime.get("accounting"), Mapping) else {}
        )
        ...
        reason = degraded_reason or ("PROVIDER_CALL_FAILED" if provider_failed else None)
        return _canonical_envelope(
            request,
            data=data,
            status=availability,
