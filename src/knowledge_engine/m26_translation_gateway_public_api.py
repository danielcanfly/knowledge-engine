...
async def _answer_event_stream(
    *,
    ...
    try:
        if _sealed_kernel_trace_diagnostic_enabled():
            with sealed_trace.request_trace() as events:
                try:
...
def _sealed_kernel_trace_diagnostic_enabled() -> bool:
    return os.environ.get("M26_SEALED_KERNEL_TRACE_DIAGNOSTIC", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }
...
def _sealed_kernel_trace_payload(
    *,
    correlation_id: str,
    question_sha256: str,
    ...
    return {
        "schema_version": "m26-sealed-kernel-trace-export/v1",
        "correlation_id": correlation_id,
        "question_sha256": question_sha256,
        "terminal": terminal,
        ...
        "raw_evidence_body_recorded": False,
    }
