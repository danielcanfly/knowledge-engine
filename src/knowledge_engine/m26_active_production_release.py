...
def resolve_active_production_release(
    ...
    *,
    ...
    production_manifest_key = _required_string(pointer, "manifest_key", "production pointer")
    production_manifest_sha256 = _required_sha256(pointer, "manifest_sha256", "production pointer")
    ...
    if not production_manifest_key.startswith(f"releases/{release_id}/promotion/"):
        raise ActiveProductionReleaseError("production manifest key is outside promotion namespace")

    production_manifest_bytes = _get_required(store, production_manifest_key, "production manifest")
    ...
    if authority.get("production_pointer_authorized") is not True:
        raise ActiveProductionReleaseError("production manifest is not pointer-authorized")

    promotion = _mapping(production_manifest.get("production_promotion"), "production promotion")
    ...
    candidate_manifest_bytes = _get_required(store, candidate_manifest_key, "candidate manifest")
    ...
    source_commit_sha = _required_git_sha(identities, "source_commit_sha", "candidate identities")
    admission_sha256 = _required_sha256(identities, "admission_sha256", "candidate identities")
    counts = _mapping(candidate_manifest.get("counts"), "candidate counts")
    semantic_point_count = _required_positive_int(counts, "semantic_documents", "candidate counts")
...
def _validate_production_artifact_family(
    *,
    ...
    if set(production) != set(candidate):
        raise ActiveProductionReleaseError("production/candidate artifact family mismatch")
...
def _artifacts_by_kind(
    ...
    for raw in artifacts:
        ...
        if expected_bytes is not None and (
            ...
            raise ActiveProductionReleaseError(f"release artifact bytes invalid: {kind}")
        result[kind] = entry
    return result
