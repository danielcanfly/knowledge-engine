# M23.7 R3.8.26 Worker-Present Seal Reconciliation

## Run 29614444848

This reconciliation independently accepts the worker-present recovery seal from
PR #889 for recovery probe run `29614444848`, covering retained diagnostic
worker `knowledge-engine-r3-8-29613277172`.

The accepted seal merged at `d3853a35d28bdb6c5285c2b5bd2745ef07021c06`.
Its exact head `9b30f94e6753c448134f393793cd9a3d6ad48dfb` passed CI, M17,
M18, and the dedicated worker-present recovery seal workflow. The accepted
seal digest is
`40374f8892c2d3670923ee847831e4311650e13081ad19e57e147e74a9e7c62d`.

The reconciled fact remains worker-present, not worker-clean. Cloudflare
returned four version identities and four deployment identities for the retained
diagnostic worker. The recovery artifact ZIP SHA-256 is
`76c5ac1ae5d202f14831ba77e672ea5e757c5259fd34a276c2407cb970d15d6c`, and the
receipt self-digest is
`cafb91201e34cc9b7c94e0b3a11c803a283b7f27369c40cd59366f649481142d`.

This reconciliation performs no worker deletion, deployment, secret mutation,
route invocation, Qdrant access, R2 access, protected mutation, blocker
clearance, fresh observation, promotion, parent closure, or M23.7 closure.

Production retrieval remains `lexical`. Retrieval quality and latency blockers
remain retained. The next legal gate is a separate exact deletion authorization
for `knowledge-engine-r3-8-29613277172`.

## Run 29611245813

This reconciliation independently accepts the worker-present recovery seal from
PR #880 for recovery probe run `29611245813`, covering retained diagnostic
worker `knowledge-engine-r3-8-29610393567`.

The accepted seal merged at `734c161dfbce906835a695fe717bb039792f0aa1`.
Its exact head `4235b0ded72f6c68ba80ca17fbbeb0b3579c2993` passed CI, M17,
M18, and the dedicated worker-present recovery seal workflow. The accepted
seal digest is
`3d8fc558375e7b27a6930cfa2490e045885b66f1b2e2ce80c0f45acbe68942a1`.

The reconciled fact remains worker-present, not worker-clean. Cloudflare
returned four version identities and four deployment identities for the retained
diagnostic worker. The recovery artifact ZIP SHA-256 is
`0622e7296ac3f7b59e3cd46074c7abd31e4096cddfc679b29242da0cea0c1dad`, and the
receipt self-digest is
`679f8e2fbdaf0a139cdb5e360d4104523ea9257e2a9d3011573b49fbe02ca429`.

This reconciliation performs no worker deletion, deployment, secret mutation,
route invocation, Qdrant access, R2 access, protected mutation, blocker
clearance, fresh observation, promotion, parent closure, or M23.7 closure.

Production retrieval remains `lexical`. Retrieval quality and latency blockers
remain retained. The next legal gate is a separate exact deletion authorization
for `knowledge-engine-r3-8-29610393567`.

## Run 29608526669

This reconciliation independently accepts the worker-present recovery seal from
PR #871 for recovery probe run `29608526669`, covering retained diagnostic
worker `knowledge-engine-r3-8-29607698618`.

The accepted seal merged at `fae5269c5d3672ff502748fa3ae7e9f20230d206`.
Its exact head `3775185eb8a5387f74c3008206e46c9e1ea7c3ed` passed CI, M17,
M18, and the dedicated worker-present recovery seal workflow. The accepted
seal digest is
`fea02c64c22ff1682c91c207e432c922b562625c17c36015b5bb957b3b92cbe1`.

The reconciled fact remains worker-present, not worker-clean. Cloudflare
returned four version identities and four deployment identities for the retained
diagnostic worker. The recovery artifact ZIP SHA-256 is
`859d6d49a5167a8445ac995e4f4ba04dd5b0693c274ebaebd6fdb9dd02d3ab12`, and the
receipt self-digest is
`62432f713ec0de751505697ff5002b75431839195e61eba6e16d5c34a1a11f09`.

This reconciliation performs no worker deletion, deployment, secret mutation,
route invocation, Qdrant access, R2 access, protected mutation, blocker
clearance, fresh observation, promotion, parent closure, or M23.7 closure.

Production retrieval remains `lexical`. Retrieval quality and latency blockers
remain retained. The next legal gate is a separate exact deletion authorization
for `knowledge-engine-r3-8-29607698618`.

## Run 29605835592

This reconciliation independently accepts the worker-present recovery seal from
PR #862 for recovery probe run `29605835592`, covering retained diagnostic
worker `knowledge-engine-r3-8-29604923286`.

The accepted seal merged at `f5be4f2026af57132c73c7beaed208f77ea5bcfe`.
Its exact head `45b58d187fec1c9a6112a364dbd7374752864725` passed CI, M17,
M18, and the dedicated worker-present recovery seal workflow. The accepted
seal digest is
`f14513676f6fdf10df67b81fc2086f9c96b94d838cd0e81e960ec81403894fa0`.

The reconciled fact remains worker-present, not worker-clean. Cloudflare
returned four version identities and four deployment identities for the retained
diagnostic worker. The recovery artifact ZIP SHA-256 is
`5ef21245a4761bbc3494c290d62abe7cd03359afb03d4a0260e7e34ef400f4c9`, and the
receipt self-digest is
`6a778e51cf82b2d50789d784adce66686bf2fe8e307a36a9377d5aeb0b35230f`.

This reconciliation performs no worker deletion, deployment, secret mutation,
route invocation, Qdrant access, R2 access, protected mutation, blocker
clearance, fresh observation, promotion, parent closure, or M23.7 closure.

Production retrieval remains `lexical`. Retrieval quality and latency blockers
remain retained. The next legal gate is a separate exact deletion authorization
for `knowledge-engine-r3-8-29604923286`.

## Run 29555974732

This reconciliation independently accepts the worker-present recovery seal from
PR #683 for recovery probe run `29555974732`, covering retained diagnostic
worker `knowledge-engine-r3-8-29553221650`.

The accepted seal merged at `9ed31c01cbe0fe343115d07f6ce3e836a146d36f`.
Its exact head `9701f69b3802091b31011e51a756035bfa313ed0` passed CI, M17,
M18, and the dedicated worker-present recovery seal workflow. The accepted
seal digest is
`9a5e154c9129ec76077bf87553bfa95246a6f302db025382087e00623174ecae`.

The reconciled fact remains worker-present, not worker-clean. Cloudflare
returned four version identities and four deployment identities for the retained
diagnostic worker. The recovery artifact ZIP SHA-256 is
`bbb6768cc0a74ee4c6487629bbd86fc085d5a76d23ae1f302d16ef8baaed3412`, and the
receipt self-digest is
`09d2bc430b4343bcd5ce03e89a718339336dcd40f8fcabc8022f444c36440ff1`.

This reconciliation performs no worker deletion, deployment, secret mutation,
route invocation, Qdrant access, R2 access, protected mutation, blocker
clearance, fresh observation, promotion, parent closure, or M23.7 closure.

Production retrieval remains `lexical`. Retrieval quality and latency blockers
remain retained. The next legal gate is a separate exact deletion authorization
for `knowledge-engine-r3-8-29553221650`.
