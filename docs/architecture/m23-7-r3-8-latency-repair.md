...
R3.8 does not lower that threshold and does not treat a rerun as a repair. It restores the execution boundary already authorised by parent #474: a transient Cloudflare Worker placed by the Qdrant hostname, using an in-process Workers AI binding and one read-only Qdrant query batch.
...
3. one Qdrant read-only query batch containing the 24 query vectors and an
...
R3.8 authorises one isolated diagnostic Worker deployment, Workers AI binding calls, read-only Qdrant collection and batch-query requests, one authenticated observation, evidence metadata and later deletion of only that exact Worker.
