...
3. one Qdrant read-only search batch containing the 24 query vectors and an
   explicit `section_id`-only payload-field allowlist;
4. only if that batch endpoint is unavailable, one read-only filtered scroll
   with the same payload allowlist and `with_vector: ["default"]`;
...
R3.8 authorises one isolated diagnostic Worker deployment, Workers AI binding calls, read-only Qdrant collection reads, one read-only search-batch attempt, at most one read-only scroll fallback after batch unavailability, one authenticated observation, evidence metadata and later deletion of only that exact Worker.
