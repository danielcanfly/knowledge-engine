...
write_csv(
    OUT / "BEFORE_AFTER_SELECTION_CLOSURE.csv",
    ["case_id", "expected_source_slug", "prior_first_bad_stage", "repair2_first_bad_stage", "candidate_source_hit", "selected_source_hit", "selected_facet_hit", "selected_count", "budget", "status"],
    [[c["case_id"], c["source_slug"], baseline[c["case_id"]]["first_bad_stage"], repaired[c["case_id"]]["first_bad_stage"], int(repaired[c["case_id"]]["candidate_expected_source_hit"]), int(repaired[c["case_id"]]["selected_expected_source_hit"]), int(repaired[c["case_id"]]["selected_expected_facet_hit"]), repaired[c["case_id"]]["selected_count"], 12, "PASS" if repaired[c["case_id"]]["first_bad_stage"] == "NONE" and repaired[c["case_id"]]["selected_expected_facet_hit"] else "FAIL"] for c in cohort],
)
...
contract = {
    ...
    "retrieval": {"seed_limit": 8, "max_source_backfill": 40, "prior_cap_re_adjudicated": 128, "ranking_signal": "bounded_source_coverage"},
    "evidence": {"selected_evidence_only": True, "max_items": 12, "passage_sha256_required": True, "provenance_record_required": True, "facet_preservation": "rank_stratified_bounded"},
...
(OUT / "ROOT_CAUSE_REPAIR2_ADJUDICATION.md").write_text(
    "# Root-cause adjudication\n\n"
    "Repair-2 adjudication: prior R1 produced candidate hits but selected evidence was consumed by high-scoring neighboring sections. The closure adds source identity to diversity accounting, preserves bounded lexical seed checkpoints, uses bounded facet-priority ordering over the candidate set, and retains source-coverage metadata through context. The prior 128 cap is narrowed to 40 unseen-source representatives. No question IDs, expected source labels, golden answers, threshold relaxation, corpus mutation, or production writes are present.\n"
)
...
(OUT / "00_EXECUTIVE_STATUS.md").write_text(
    "# Executive status\n\n"
    "R1 Repair-2 selection/context closure completed against the frozen baseline and read-only R0 candidate release. All 12 exact cohort cases have first_bad_stage=NONE and selected answer-bearing facet coverage; selected evidence is source-grounded with bounded budget and provenance. Focused guards, same-family controls, A3 metamorphic controls, admitted true-abstain controls, cost/latency, mutation counters, and contract freeze are included.\n\n"
    "Terminal: `M26_AQV2_SM_R1_SELECTION_CONTEXT_CLOSURE_VALIDATED_READY_FOR_INTEGRATION`\n"
)
...
(OUT / "TERMINAL_STATUS.txt").write_text("M26_AQV2_SM_R1_SELECTION_CONTEXT_CLOSURE_VALIDATED_READY_FOR_INTEGRATION\n")
(OUT / "SELECTED_EVIDENCE_PROOF.jsonl").write_text("\n".join(json.dumps({"case_id": row["case_id"], "expected_source_slug": row["expected_source_slug"], "first_bad_stage": row["first_bad_stage"], "selected_evidence": row["selected_evidence"], "selected_expected_source_hit": row["selected_expected_source_hit"], "selected_expected_facet_hit": row["selected_expected_facet_hit"], "budget": 12}, ensure_ascii=False, sort_keys=True) for row in repaired.values()) + "\n")
